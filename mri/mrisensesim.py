...
This function simulates an array of MR sensitivity coils. The function arranges
the coil geometry such that it follows the bird cage design (i.e., rows of
rings). Some customization is possible depending on user parameters.
"""
import numpy as np
...
def main():
    ...
    sys.path.append('./')
    import visual.view_vol as view_vol

    size = (64, 64)

    smap = np.array(mrisensesim(size))

    plt.gray()
    view_vol.multi_slice_viewer(np.absolute(smap))
